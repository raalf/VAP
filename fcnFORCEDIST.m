...
% matMOMDIST = liftfree.*matSCLST + liftind.*matSCLST;
...
for i = 1:vecM(1)
    ...
    row_ledves(:,:,i) = [matNPVLST(matNPDVE(matROWS(:,i),1),:); matNPVLST(matNPDVE(matROWS(end,i),2),:)];
    ...
    temp_lift = [temp_lift(1); temp_liftdist'];
    % Why these two lines can't be done all in one is beyond me but MATLAB
    % did not like it
    lift_moment(:,:,i) = [temp_lift.*matLIFTDIR(matROWS(:,i),:); zeros(1,3)]; % Lift force in X, Y, Z components to be used in cross product to calculate moment about elastic axis
...
tempMOMDIST = cross(delX,lift_moment);
