function [matEIx, matGJt, vecEA, vecCG, vecJT, vecLM, vecLSM, vecLSAC, matAEROCNTR, matSCLST, vecSPANDIST, matSC, vecMAC, vecLAMBDA] = fcnIMPSTRUCTDIST(vecDVEHVSPN, vecDVELE, vecDVETE, vecEIxCOEFF, vecGJtCOEFF,...
...
matQTRCRD = fcnSTARGLOB([0.25*(2*vecDVEHVCRD),zeros(length(vecDVEHVCRD),2)],vecDVEROLL(matROWS(:,1)),vecDVEPITCH(matROWS(:,1)),vecDVEYAW(matROWS(:,1)));

tempLE = (matNPVLST(matNPDVE(ledves,1),:) + matNPVLST(matNPDVE(ledves,2),:))./2; % DVE LE coordinates (at mid-point of DVE)
...
vecLAMBDA = [matCENTER(ledves(1),2)/(valSPAN/2); vecLAMBDA];

%% Structural Properties

% Spanwise bending stiffness distribution. Cols 2 and 3 are the first and
% second derivatives
matEIx(:,1) = (vecEIxCOEFF(1).*matCENTER(ledves,2).^2 + vecEIxCOEFF(2).*matCENTER(ledves,2) + vecEIxCOEFF(3))';
matEIx(:,2) = (2*vecEIxCOEFF(1).*matCENTER(ledves,2) + vecEIxCOEFF(2))';
matEIx(:,3) = (repmat(2*vecEIxCOEFF(1),1,size(matCENTER,1)))';

% Spanwise torsional stiffness distribution. Col 2 is the first derivative
matGJt(:,1) = (vecGJtCOEFF(1).*matCENTER(ledves,2).^2 + vecGJtCOEFF(2).*matCENTER(ledves,2) + vecGJtCOEFF(3))';
matGJt(:,2) = (2*vecGJtCOEFF(1).*matCENTER(ledves,2) + vecGJtCOEFF(2))';

vecEA = vecEACOEFF(1).*matCENTER(ledves,2).^2 + vecEACOEFF(2).*matCENTER(ledves,2) + vecEACOEFF(3);
vecCG = vecCGCOEFF(1).*matCENTER(ledves,2).^2 + vecCGCOEFF(2).*matCENTER(ledves,2) + vecCGCOEFF(3);
vecJT = vecJTCOEFF(1).*matCENTER(ledves,2).^2 + vecJTCOEFF(2).*matCENTER(ledves,2) + vecJTCOEFF(3);
vecLM = vecLMCOEFF(1).*matCENTER(ledves,2).^2 + vecLMCOEFF(2).*matCENTER(ledves,2) + vecLMCOEFF(3);
...
matCG = fcnSTARGLOB(tempCG,vecDVEROLL(matROWS(:,1)),vecDVEPITCH(matROWS(:,1)),vecDVEYAW(matROWS(:,1))); % Transform to global coordinates

matSC = fcnSTARGLOB(tempEA,vecDVEROLL(matROWS(:,1)),vecDVEPITCH(matROWS(:,1)),vecDVEYAW(matROWS(:,1))); % Transform to global coordinates
