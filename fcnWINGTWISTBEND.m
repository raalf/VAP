...
vecMASS = vecLAMBDA.*valSPAN.*vecLM./2;
...
b_01_0n(1) = b_01_0n(1) + 4*matEIx(1,1)/(vecLAMBDA(1)*valSPAN/2)^3;
b_01_0n(2) = b_01_0n(2) - matEIx(1,1)/(vecLAMBDA(1)*valSPAN/2)^3;
...
if valTIMESTEP == 1
    ...
    % Response at n
    matRES = (matD + S2 + 8*S3)\[vecLIFTDIST.*(vecLAMBDA.*valSPAN/2) - vecMASS*9.81; vecMOMDIST.*(vecLAMBDA.*valSPAN/2) - vecMASS*9.81.*vecLSM];
    ...
    vecQ = S1*[matDEF(end,:)'; matTWIST(end,:)'] + S2*[matDEF(end-1,:)'; matTWIST(end-1,:)'] + S3*[matDEF(end-2,:)'; matTWIST(end-2,:)'] + [vecLIFTDIST.*(vecLAMBDA.*valSPAN/2) - vecMASS*9.81; vecMOMDIST.*(vecLAMBDA.*valSPAN/2) - vecMASS*9.81.*vecLSM];
