...
if valTIMESTEP > valMAXTIME - max(vecAZNUM) && flagVISCOUS == 1% Only run for last rotation
    ...
    vecDVEROTOR, matUINF, matVLST, matDVE, induced_vel);
%     [vecCLPDIST, vecCDPDIST, Pthrust, Ptorque, difthrustP, diffsideP, diffaxialP] = fcnRVISCOUS(en, es, ea,  flagVERBOSE, ...
%         vecRPM, valDIA, valKINV, vecQARM, vecDVEHVCRD, vecN, vecM, ...
%         vecDVELE, vecDVEPANEL, vecAIRFOIL, vecTHETA, vecDISNORM, vecDVEAREA,...
%         matUINF, matVLST, matDVE, induced_vel);

%     [difthrustP, diffaxialP] = fcnRVISCOUS2(flagVERBOSE, vecRPM, valKINV, vecQARM, vecTHETA, vecDVEAREA, vecAIRFOIL, vecDVEHVCRD, vecN, vecM, vecDISNORM, vecDVELE, vecDVEPANEL, matVLST, matDVE, matUINF, induced_vel);
%     vecCDPDIST = [];
%     vecCLPDIST = [];
