function [aloc, bloc, cloc] = fcnVSINDCODIST(hspan, hchord, phi, fp_0, k)
...
% eta_0 = distributed(eta_0);
% xsi_0 = distributed(xsi_0);
% zeta_0 = distributed(zeta_0);
% hspan = distributed(hspan);
% hchord = distributed(hchord);
% phi = distributed(phi);
% k = distributed(k);
% b23 = distributed(zeros(len,1));
% b25 = distributed(-ones(len,1));
% % b26 = distributed(zeros(len,1));
% % b27 = distributed(zeros(len,1));
% c27 = distributed(repmat(2,len,1));
% % b2_xsi = distributed(zeros(len,1));
% % c2_xsi = distributed(zeros(len,1));
% % G21b = distributed(zeros(len,1));
% % G21c = distributed(zeros(len,1));
% b26 = b23;
% b27 = b23;
% b2_xsi = b23;
% c2_xsi = b23;
...
% xsi_0 = fp_0(:,1);
% zeta_0 = fp_0(:,3);
eta_0 = codistributed(eta_0);
xsi_0 = codistributed(xsi_0);
zeta_0 = codistributed(zeta_0);
hspan = codistributed(hspan);
hchord = codistributed(hchord);
phi = codistributed(phi);
k = codistributed(k);
b23 = codistributed(zeros(len,1));
% b25 = codistributed(-ones(len,1));
% b26 = distributed(zeros(len,1));
% b27 = distributed(zeros(len,1));
% c27 = codistributed(repmat(2,len,1));
% b2_xsi = distributed(zeros(len,1));
% c2_xsi = distributed(zeros(len,1));
% G21b = distributed(zeros(len,1));
% G21c = distributed(zeros(len,1));
...
b21 = -le_vect;
b22 = (zeta_0sq).*tanphi;
% b23 = distribute(zeros(len,1));
b24 = -tanphi;
% b25 = distribute(-ones(len,1));
% b26 = distribute(zeros(len,1));
% b27 = distribute(zeros(len,1));
