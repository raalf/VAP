function [valCT, valCQ, valCP] = fcnRVISCOUS(flagVERBOSE, valCT, valCQ, ...
    valCP, valRPM, valDIA, valKINV, vecCPRADI, vecDVEHVCRD, vecN, vecM, ...
...
% CN = 2*(N/rho)/(V^2*S)
vecCNDIST = (sum(vecDISNORM(rows),2).*2)./(sum(vecDVEAREA(rows),2).*(vecV(rows).^2));
vecREDIST = vecV.*2.*sum(vecDVEHVCRD(rows),2)./valKINV;

vecCDPDIST = zeros(len,1);
len = 0;
...
vecDPDIST = 0.5*(vecCDPDIST.*((vecV(rows)).^2).*(sum(vecDVEAREA(rows),2)));

% Apply an appropriate directions
tempDIR = [matUINF(:,1).*cos(vecTHETA) matUINF(:,2).*sin(vecTHETA) matUINF(:,3)];
...
TORQUEDIST = (dot(matDPDIST,[abs(cos(vecTHETA)) abs(sin(vecTHETA)) zeros(size(matDPDIST,1),1)],2)).*vecCPRADI;
